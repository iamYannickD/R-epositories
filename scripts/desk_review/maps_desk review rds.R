...
High_risk	<- c("Algeria", "Angola", "Benin", "Burkina Faso", "Cameroon", "Central African Republic", "Cote d Ivoire", 
...
afroCountries <- read_rds("../data/global.ctry.rds") |>
  filter(WHO_REGION == 'AFRO')
afroProvinces <- read_rds("../data/global.prov.rds") |>
  filter(WHO_REGION == 'AFRO')
afroDistricts <- read_rds("../data/global.dist.rds") |>
  filter(WHO_REGION == 'AFRO')
...
country <- afroCountries$ADM0_NAME |>
  unique() |> sort()
