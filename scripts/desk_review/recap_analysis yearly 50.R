...
# load data
es_sites <- read_csv("../data/data_dr/es_sites/ES_performance_from_2025-01-01_to_2025-06-31.csv") 
...
Low_risk <- c("Cabo Verde", "Eswatini", "Lesotho", "Mauritius", "Seychelles")
...
recap_es_sites <- 
  es_sites |>
  dplyr::select(Countryname = Country, EV_isolation_Rate_Q1, EV_isolation_Rate_Q2) |> #, EV_isolation_Rate_9m, EV_isolation_Rate_12m
    left_join(y = risk_level_by_country, by = c("Countryname" = "country")) |>
    filter(!is.na(risk_level)) |>
    group_by(risk_level, Countryname) |>
    mutate(
      country_Q1 = if_else(!is.na(EV_isolation_Rate_Q1), 1, 0),
      country_Q1_50ev = if_else( (!is.na(EV_isolation_Rate_Q1) & EV_isolation_Rate_Q1  >= 0.5) , 1, 0),
      country_Q1_80ev = if_else( (!is.na(EV_isolation_Rate_Q1) & EV_isolation_Rate_Q1  >= 0.8), 1, 0),
      
      country_Q2 = if_else(!is.na(EV_isolation_Rate_Q2), 1, 0),
      country_Q2_50ev = if_else( (!is.na(EV_isolation_Rate_Q2) & EV_isolation_Rate_Q2  >= 0.5) , 1, 0),
      country_Q2_80ev = if_else( (!is.na(EV_isolation_Rate_Q2) & EV_isolation_Rate_Q2  >= 0.8), 1, 0),
    ...
    summarise(
      `%Q1countries >= 50` = 100 * sum(country_Q1_50ev, na.rm = TRUE) / sum(country_Q1, na.rm = TRUE),
      `%Q2countries >= 50` = 100 * sum(country_Q2_50ev, na.rm = TRUE) / sum(country_Q2, na.rm = TRUE),
      ...
      `%Q1countries >= 80` = 100 * sum(country_Q1_80ev, na.rm = TRUE) / sum(country_Q1, na.rm = TRUE),
      `%Q2countries >= 80` = 100 * sum(country_Q2_80ev, na.rm = TRUE) / sum(country_Q2, na.rm = TRUE)
    ...
    mutate(
      #sparkline_50 = map2( `%Q1countries >= 50`, `%Q2countries >= 50`, ~ list(c(.x, .y))),
          sparkline_50 = map2( `%Q1countries >= 50`, `%Q2countries >= 50`, ~ list(c(.x, .y))), # `%Q3countries >= 50`, `%Q4countries >= 50`
          sparkline_80 = map2( `%Q1countries >= 80`, `%Q2countries >= 80`, ~ list(c(.x, .y))) # `%Q3countries >= 80`, `%Q4countries >= 80`,
    ...
    cols_label(
      "%Q1countries >= 50" = "Q1, 2025",
      "%Q2countries >= 50" = "Q2, 2025",
      # "%Q3countries >= 50" = "Q2, 2024",
      # "%Q4countries >= 50" = "Q1, 2024",
      "%Q1countries >= 80" = "Q1, 2025",
      "%Q2countries >= 80" = "Q2, 2025",
    ...
    tab_header(
      title = md("**Summary of ES site sensitivity, Q1 & Q2, 2025**"),
    ...
    cols_align(
      align = "center",
      columns = c(`%Q1countries >= 50`, `%Q2countries >= 50`, #`%Q3countries >= 50`, `%Q4countries >= 50`,
                  `%Q1countries >= 80`, `%Q2countries >= 80`, nanoplots_50) #`%Q3countries >= 80`, `%Q4countries >= 80`,
    ) |>
...
recap_es_sites
  ...
  #save as a png file
  gtsave(recap_es_sites, paste0("../data/data_dr/outputs/Recap/", "/recap_es_desk_review.png"), expand = 20
         , vheigh = 20)
