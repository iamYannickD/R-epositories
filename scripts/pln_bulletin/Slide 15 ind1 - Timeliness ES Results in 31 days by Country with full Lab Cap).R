 ...
 EStables2025 |> 
  ...
  summarize(
    ...
    ITD_results_21days = sum( (str_detect(Finalcellcultureresult, "^1") | str_detect(Finalcellcultureresult, "^4")) & 
                                !is.na(FinalcombinedrRTPCRresults) & (date_itd_result - Dateofcollection) < 32 & 
                                (date_itd_result - Dateofcollection) >= 0, na.rm = TRUE),
    Prop_ITD_31days = round(ITD_results_21days / ITD_results * 100, 0),
  ) |>
  select(IST, Countrycode, Prop_ITD_31days) |>
  filter(!is.na(Prop_ITD_31days), Prop_ITD_31days > 0) |>
  pivot_longer(cols = c(Prop_ITD_31days), names_to = "Proportions", values_to = "Values") |>
  ...
  scale_fill_manual(
    values = c("Prop_ITD_31days" = "gold", "Prop_ITD_31days" = "darkblue"),
    labels = c("Prop_ITD_31days" = "Among all samples (with results)", "Prop_ITD_31days" = "Among positive samples")
  ) +
  scale_fill_manual(
...
ggsave("ESCountry31_plot.png", ES_byCountry31, path = "../data/outputs/", width = 13, height= 6)  
