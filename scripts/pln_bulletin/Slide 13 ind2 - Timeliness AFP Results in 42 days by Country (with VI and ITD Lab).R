...
AFPCountries_42p <- 
 AFPtables |>
  ...
  filter( (CountryCode %in% c("ALG", "CAE", "CAF", "CIV", "ETH", "NIE", "KEN", "MAD", "RDC", "SEN", "UGA", "ZAM", "ZIM")) ) |>
  mutate(workload_by_lab = n(),
         time_itd_results_42days = as.numeric(difftime(proxy_date_itd_result, DateOfOnset, units = "days")),
         
         is_itd = if_else( (FinalCellCultureResult %in% c("1-Suspected Poliovirus", "4-Suspected Poliovirus + NPENT")), 1, 0),
         
         is_itd_42days = if_else( (FinalCellCultureResult %in% c("1-Suspected Poliovirus", "4-Suspected Poliovirus + NPENT")) &
                                    (!is.na(FinalITDResult) & time_itd_results_42days < 43 & time_itd_results_42days >= 0), 1, 0)
  ) |>
  summarize(
    ITD_results = sum(is_itd, na.rm = TRUE),
    ITD_results_42days = sum(is_itd_42days, na.rm = TRUE),
    Prop_ITD_42days = 100 * ITD_results_42days / ITD_results,
  ) |>
  filter(!is.na(Prop_ITD_42days) & Prop_ITD_42days > 0) |>
  # For intermediary results
  #group_by(IST) |>  summarize(median_Prop_ITD_42days = median(Prop_ITD_42days, na.rm = TRUE)) #to know the proportion 42 days by IST
  dplyr::select(IST, CountryCode, Prop_ITD_42days)  |>
  pivot_longer(
  ...
  scale_fill_manual(
    values = c("Prop_ITD_42days" = "gold"),
    labels = c("Prop_ITD_42days" = "Among all samples (with results)")
  ) +
  scale_fill_manual(
...
ggsave("AFPCountries_42_plot.png", AFPCountries_42p, path = "../data/outputs/", width = 13, height= 6) 
