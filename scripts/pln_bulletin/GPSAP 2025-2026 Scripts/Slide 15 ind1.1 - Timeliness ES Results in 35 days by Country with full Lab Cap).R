...
data_ES <- seq_result_ES |>
  mutate(
    CountryCode = substr(EPID.NUMBER, start = 5, stop = 7),
    .before = COUNTRY
  ) |>
  mutate(
    IST = case_when(
...
Sequencing.results.countries.with.labs + geom_label(
                    data = summary_data,
