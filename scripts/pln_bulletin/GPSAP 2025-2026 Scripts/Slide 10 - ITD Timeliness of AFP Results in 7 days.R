...
AFPtables <- DBI::dbGetQuery(AFPdb, "SELECT * FROM POLIOLAB ORDER BY LabName, EpidNumber;", stringsAsFactors = FALSE) |>
  tibble() |>  mutate(proxy_date_infor_itd = coalesce(ymd(DateIsolateinforITD), ymd(DateLarmIsolateRec),
                                                      ymd(DateRarmIsolateSentforITD), ymd(DateFinalCellCultureResults)),
...
AFPtables |>
  ...
  mutate(workload_by_lab = n(),
         time_itd_results_7days = as.numeric(difftime(proxy_date_itd_result, proxy_date_infor_itd, units = "days")),
         ...
         is_itd_7days = if_else( (FinalCellCultureResult %in% c("1-Suspected Poliovirus", "4-Suspected Poliovirus + NPENT")) &
                                 (!is.na(FinalITDResult) & time_itd_results_7days < 8 & time_itd_results_7days >= 0), 1, 0),
         is_itd_7days_positive_sample = if_else( (FinalCellCultureResult %in% c("1-Suspected Poliovirus", "4-Suspected Poliovirus + NPENT")) &
  ...
  summarize(
    ...
    ITD_results_7days_positive_sample = sum(is_itd_7days_positive_sample, na.rm = TRUE),
    Prop_ITD_7days = 100 * ITD_results_7days / ITD_results,
    Prop_ITD_7days_positive_sample = 100 * ITD_results_7days_positive_sample / ITD_results_positive_sample
        ) |>
  # Intermediary results
    # summarise(
    #   prop_lab_res_7days_all_samples = sum(ITD_results_7days, na.rm = TRUE) / sum(ITD_results, na.rm = TRUE),
    #   prop_lab_res_7days_positive_samples = sum(ITD_results_7days_positive_sample, na.rm = TRUE) / sum(ITD_results_positive_sample, na.rm = TRUE))

    dplyr::select(LabName, Prop_ITD_7days, Prop_ITD_7days_positive_sample)  |>
    pivot_longer(
...
ggsave("AFPtables21_plot.png", AFPtables_7, path = "../data/outputs/", width = 12, height= 6)   
